...
mod tests {
    ...
    fn rust_analyzer_reports_virtual_document_diagnostics_and_stops() {
        let _guard = RUST_ANALYZER_TEST_LOCK.lock().unwrap();
        let workspace = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .parent()
            .and_then(Path::parent)
            .unwrap()
            .to_owned();
        let path = workspace.join("crates/lsp/src/lib.rs");
        let backend = LocalLspBackend::default();
        if let Err(error) = backend.start(&workspace) {
    ...
    fn rust_analyzer_completes_and_finds_virtual_document_definition() {
        ...
        let backend = LocalLspBackend::default();
        backend.start(&workspace).unwrap();
